...
sum_individual_rewards_and_fees as (
    select 
        ...
        sum(dps.network_fee_amount) as sum_network_fee_wei,
        --sum(dps.protocol_fee_amount) as sum_protocol_fee_amount_wei, todo check with felix if we ignore this column
...
sum_cow_rewards_and_fees as (
    select 
        ...
        coalesce(sum_slippage_wei, 0) + coalesce(sum_network_fee_wei, 0) as total_native_transfer
