...
old_order_data as (
    select 
        ...
        protocol_fee_token as protocol_fee_token_old,
        nullif(replace(replace(quote_buy_amount::text, '"', ''), '\', ''), 'null')::numeric as quote_buy_amount_old,
        quote_gas_cost as quote_gas_cost_old, 
        nullif(replace(replace(quote_sell_amount::text, '"', ''), '\', ''), 'null')::numeric as quote_sell_amount_old,
        quote_sell_token_price as quote_sell_token_price_old, 
        quote_solver as quote_solver_old,
        solver as solver_old,
        nullif(replace(replace(surplus_fee::text, '"', ''), '\', ''), 'null')::numeric as surplus_fee_old,
        tx_hash as tx_hash_old
